...
c     This subroutine prints the elements computed previously  
      ...
      string='1. Integration of detachment and attachment densities'
      write(6,'(a53)') string
      write(6,*) " " 
      string='Integral of detachment density '
      write(6,'(a52,f18.2)') string, sinteg1
      write(6,*) " " 
      string='Integral of attachment density '
      write(6,'(a52,f18.2)') string,sinteg2
      write(6,*) " " 
      string='2. Charge from p-/p+ densities'
      write(6,'(a52)') string
      write(6,*) " " 
      string='chi- from negative density variation is '
      write(6,'(a52,f18.2)') string , (-1)*sintegm
      write(6,*) " " 
      string='chi+ from positive density variation is '
      write(6,'(a52,f18.2)') string,sintegp
      write(6,*) " " 
      string='chi and phi indices value'
      write(6,'(a52,f18.2,f13.2)') string,0.5d0*(sintegp-sintegm),
     ...
     $           srct,srct*ata
      write(6,*) " " 
      string='4. Centroid coordinates (bohr) from D/A, zeta'
      write(6,'(a52)') string
      write(6,*) " " 
      string='Detachment density centroid coordinates'
      write(6,'(a60,3f13.5)') string,
     $       sx1c, sy1c, sz1c
      write(6,*) " " 
      string='Attachment density centroid coordinates'
      write(6,'(a60,3f13.5)') string,
     $        sx2c, sy2c, sz2c
      write(6,*) " " 
      string='CT distance from D/A in Bohr and Angstrom'
      write(6,'(a60,2f13.5)') string,
     $           sr12ct,sr12ct*ata
      write(6,*) " " 
      string='5. phi_S (D/A overlap) and psi indices calculation'
      write(6,'(a60)') string
      write(6,*) " " 
      string='phi_S index value is'
      write(6,'(a60,f10.2)') string,
     $ sphis
      write(6,*) " " 
      string='psi index value is'
